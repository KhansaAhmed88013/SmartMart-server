...
// Discounts Models
// =======================
const ItemsDiscount = sequelize.define('ItemsDiscount', {
  id: { type: DataTypes.INTEGER, autoIncrement: true, primaryKey: true },
...
const BillDiscount = sequelize.define('BillDiscount', {
  id: { type: DataTypes.INTEGER, autoIncrement: true, primaryKey: true },
  conditionType: { type: DataTypes.ENUM('Above', 'EqualOrAbove', 'Below'), allowNull: false },
  amount: { type: DataTypes.DECIMAL(65, 2), allowNull: false },
...
const CategoryDiscount = sequelize.define('CategoryDiscount', {
  id: { type: DataTypes.INTEGER, autoIncrement: true, primaryKey: true },
  category_id: { type: DataTypes.INTEGER, allowNull: false, references: { model: "categories", key: "id" }, onUpdate: "CASCADE", onDelete: "CASCADE" },
...
const Invoice = sequelize.define('Invoice', {
  ...
  payment_method: { type: DataTypes.ENUM('Cash Sale', 'Credit Card', 'Credit Customer'), defaultValue: 'Cash Sale' },
...
const Purchase = sequelize.define('Purchase', {
  ...
  payment_status: { type: DataTypes.ENUM('Pending', 'Partial', 'Paid', 'Cancelled'), defaultValue: 'Pending' }
}, {
  tableName: 'purchases',
  timestamps: true,
  createdAt: 'created_at',
...
const StockLedger = sequelize.define('StockLedger', {
  ...
  transaction_type: { type: DataTypes.ENUM('Opening', 'Purchase', 'Sale', 'Return', 'Adjustment'), allowNull: false },
...
async function recordLedger({ product_id, transaction_type, transaction_id = null, qty_in = 0, qty_out = 0, remarks = null, transaction = null }) {
  ...
  // update snapshot
  await Product.update({ qty: newBal }, { where: { id: product_id }, hooks: false, transaction });
  return ledger;
}
...
const connectAndSync = async () => {
  try {
    ...
    const defaultUsername = process.env.DEFAULT_ADMIN_USERNAME || "MartAdmin";
    const defaultPassword = process.env.DEFAULT_ADMIN_PASSWORD || "admin12345";
    const defaultEmail = process.env.DEFAULT_ADMIN_EMAIL || "khansa88013@gmail.com";
    const existingUser = await User.findOne({ where: { username: defaultUsername } });

    if (!existingUser) {
      const hashedPassword = await bcrypt.hash(defaultPassword, 10);
      await User.create({ username: defaultUsername, password: hashedPassword, full_name: "Mart Admin", role: "Admin", status: "Active", email: defaultEmail });
    ...
    for (const unitName of defaultUnits) {
      const [unit, created] = await Unit.findOrCreate({ where: { name: unitName } });
      console.log(created ? `✅ Default unit created: ${unitName}` : `ℹ️ Unit already exists: ${unitName}`);
    }
...
module.exports = {
  sequelize,
  connectAndSync,
  Product, Supplier, Category, Customer, Invoice, InvoiceItem,
  ShopProfile, ItemsDiscount, Purchase, PurchaseItem, StockLedger,
  BillDiscount, User, CategoryDiscount, Unit, Notification,
  recordLedger, getLastBalance
};

/* ℹ️ transaction_id meaning:
   - For Purchase: Purchase.id
   - For Sale: Invoice.id
   - For Opening or Adjustment: usually null (or manual reference)
*/
